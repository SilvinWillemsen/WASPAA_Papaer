...
\end{equation}
where $T_\text{c}$ is the tension contributed by the core and $T_\text{w}$ the tension contributed by the wrapping. In this section we will show a way to correctly calculate Young's modulus $E$ for wrapped strings.
...
\begin{equation}
    \mu = \frac{\pi}{4}\Big(\rho_\text{c}d_\text{c}^2 + \rho_\text{w}\big[(2d_\text{w}+d_\text{c})^2-d_\text{c}^2\big]\Big), \quad [\text{kg}\cdot\text{m}],
\end{equation}
with wrapping diameter $d_\text{w}$ core and wrapping densities $\rho_\text{c}$, $\rho_\text{w}$. and using this with a desired fundamental frequency $\omega_0$ to calculate $T_0$:
\begin{equation}\label{eq:tension}
    T_0 = \mu(2L\omega_0)^2, \quad [\text{N}],
\end{equation}
Assuming now that we know $A_\text{c}$, $L$ and $T_0$ we need to obtain $\Delta L$ to calculate $E$ in \eqref{eq:tensile_stress}. It requires high precision to measure $\Delta L$ in a guitar string. It is, however, possible to calculate it from material properties. For the core of the string we can rewrite~\eqref{eq:tensile_stress}, i.e.,
%
